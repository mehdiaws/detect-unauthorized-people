...
aws s3api get-object --bucket mehdibucket1 --key app_unauthorized.zip app.zip\
unzip app.zip -d ./app\
cd ./app\
pip install -r requirements.txt\
pip install pytz -t lambda/imageprocessor/\
sed -i -e "s/<NO-DEFAULT>/\cf3 YOUR PIX BUCKET\cf2 /g" "config/global-params.json"\
